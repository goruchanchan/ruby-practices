...
class LsFile
  ...
  def file_message
    view_message(Formatter.convert_array(@names))
  end

  def file_long_message
    view_long_message(Formatter.convert_list_segment(@names, '.'))
  end

  def view_message(files)
    files.map do |column|
