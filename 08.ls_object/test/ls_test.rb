...
class LsCommandTest < Minitest::Test
  ...
  def test_directory_l_option
    ...
    TEXT
    input = Input.new(paths: %w[test/test_data], option_all: false, option_reverse: false)
    assert_equal expected, Formatter.new(groups: input.groups, option_long: true).to_s
  end
  ...
  def test_directory_a_option
    expected = <<~TEXT.chomp
      ...
      .rubocop.yml    bin
    TEXT
    input = Input.new(paths: %w[.], option_all: true, option_reverse: false)
    assert_equal expected, Formatter.new(groups: input.groups, option_long: false).to_s
  end
  ...
  def test_directory_rla_option
    ...
    TEXT
    input = Input.new(paths: %w[test/test_data/child_dir], option_all: true, option_reverse: true)
    assert_equal expected, Formatter.new(groups: input.groups, option_long: true).to_s
  end
end
